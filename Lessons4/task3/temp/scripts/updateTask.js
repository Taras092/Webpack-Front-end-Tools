...
import { getTasksLists, updateTask, deleteTask } from "./tasksGetaway.js"; // export const onToggleTask = (event) => {
...
const deleteTaskElem = target => {
  ...
  deleteTask(taskId).then(() => getTasksLists()).then(newTasksList => {
    setItem("tasksList", newTasksList);
    renderTasks();
  });
};

export const onToggleTask = _ref => {
  let {
    target
  } = _ref;
  ...
  const {
    text
  } = tasksList.find(idEl => idEl.id === taskId);
  const done = target.checked;
  const updatedTask = {
    text,
    done
  };
  updateTask(taskId, updatedTask).then(() => getTasksLists()).then(newTasksList => {
